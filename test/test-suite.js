...
export function buildTests(name, isProduction, store, makeReducer, makeStore) {
  let storeKey = (function () {
    let key = 1
    return () => key++
  }())

  function modeTest(name, devTest, prodTest) {
    if (isProduction) {
      if (prodTest) return test(`[production] ${name}`, prodTest)
    } else {
      if (devTest) return test(`[development] ${name}`, devTest)
    }
  }
  ...
  describe(name, () => {
    ...
    describe('user can customize the action being fired', () => {
      ...
      modeTest('.action should throw when passed an improper number of args', () => {
        const storeObject = makeStore({ key: storeKey() })
    ...
    describe("user can modify the store using `.set()`", () => {
      ...
      modeTest('.set should throw when passed an improper number of args', () => {
        const storeObject = makeStore({ key: storeKey() })
    ...
    describe("user can read the store", () => {
      ...
      modeTest('.get should throw when passed an improper number of args', () => {
        const storeObject = makeStore({ key: storeKey() })
